...
const WorldwideChartsGrid: React.FC<WorldwideChartsGridProps> = ({
  ...
  const rankingTitle = rankMetricLabel
    ? `${copy.worldwide.topRanking} ${rankEntityLabel} • ${rankMetricLabel}`
    : `${copy.worldwide.topRanking} ${rankEntityLabel}`;

  return (
    <div className="world-chart-grid">
      <div className="world-chart-card world-chart-card-wide">
        <div className="chart-header">
          <p className="panel-kicker">{copy.worldwide.dailyTrends}</p>
          {worldLoading ? <span className="pill pill-ghost">{copy.common.loading}</span> : null}
        </div>
        {timelineChartData.length ? (
          ...
          <div className="chart-placeholder">{copy.worldwide.noTimelineData}</div>
        )}
      </div>

      <div className="world-chart-card">
        <div className="chart-header">
          <p className="panel-kicker">{rankingTitle}</p>
          {rankingLoading ? <span className="pill pill-ghost">{copy.common.loading}</span> : null}
        </div>
        {rankLabels.length ? (
          <div className="world-plot-frame">
            <Plot
          ...
          <div className="chart-placeholder">{copy.worldwide.noRankingData}</div>
        )}
      </div>

      <div className="world-chart-card">
        <div className="chart-header">
          <p className="panel-kicker">{copy.worldwide.casesMomentum}</p>
        </div>
        {momentumDates.length ? (
          ...
          <div className="chart-placeholder">{copy.worldwide.noMomentumData}</div>
        )}
      </div>

      <div className="world-chart-card">
        <div className="chart-header">
          <p className="panel-kicker">{copy.worldwide.weekdayProfile}</p>
        </div>
        {weekdayValues.some((value) => value > 0) ? (
          ...
          <div className="chart-placeholder">{copy.worldwide.noWeekdayProfile}</div>
        )}
      </div>

      <div className="world-chart-card">
        <div className="chart-header">
          <p className="panel-kicker">{copy.worldwide.outcomeComposition}</p>
        </div>
        {outcomeItems.length ? (
          <div className="world-plot-frame">
            <Plot
          ...
          <div className="chart-placeholder">{copy.worldwide.noOutcomeData}</div>
        )}
      </div>
